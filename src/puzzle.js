...
import { D, recipeFor, label, labelL as L, dishLabel } from './game/recipes.js';
...
export class Puzzle {
  ...
  buildOrder(dish) {
    ...
    $('pzTitle').textContent = T('pz.order.title'); $('pzAnswer').replaceChildren(); $('pzAnswer').classList.remove('hidden');
    $('pzGrid').replaceChildren(...shuffle(steps.map((t, idx) => idx)).map((idx) => {
      ...
      el.onclick = () => { this.cur.taps++; if (idx === k) { el.classList.add('ok'); el.disabled = true; const a = document.createElement('span'); a.className = 'ans'; a.textContent = `${k + 1}. ${L(steps[idx])}`; $('pzAnswer').appendChild(a); k++; this.o.sfx?.place?.(); if (k === steps.length) this.solved(); } else this.wrong(el, T('pz.order.wrong', { item: L(steps[idx]), n: k + 1 })); };
      return el;
    }));
  }
  ...
  buildIntruder(dish) {
    ...
    $('pzTitle').textContent = T('pz.odd.title', { n: bad.length }); $('pzAnswer').classList.add('hidden');
    $('pzGrid').replaceChildren(...shuffle([...mine.slice(0, 8), ...bad]).map((it) => {
      ...
      el.onclick = () => { this.cur.taps++; if (bad.includes(it)) { el.classList.add('bad-found'); el.disabled = true; this.o.sfx?.trash?.(); if (--left === 0) this.solved(); } else this.wrong(el, T('pz.odd.wrong', { item: L(it), dish: dishLabel(dish) })); };
      return el;
    }));
  }
  ...
  buildMissing(dish) {
    ...
    $('pzTitle').textContent = T('pz.hidden.title'); $('pzAnswer').classList.remove('hidden');
    ...
    $('pzGrid').replaceChildren(...shuffle([answer, ...others]).map((t) => {
      ...
      el.onclick = () => { this.cur.taps++; if (t === answer) { el.classList.add('ok'); $('pzAnswer').querySelector('.hole').textContent = `${hide + 1}. ${L(answer)}`; this.solved(); } else this.wrong(el, T('pz.hidden.wrong', { item: L(t) })); };
      return el;
    }));
  }
  ...
  buildNinja(dish) {
    ...
    $('pzTitle').textContent = T('pz.ninja.title', { dish: dishLabel(dish), n: good.length }); $('pzAnswer').classList.add('hidden'); $('pzGrid').classList.add('hidden');
    ...
    this.ninja = new Ninja(cv, { good, bad, onHit: (ok, tok) => { this.cur.taps++; if (!ok) this.wrong(cv, T('pz.notIn', { item: L(tok), dish: dishLabel(dish) })); else { this.o.sfx?.place?.(); $('pzMsg').textContent = ''; } }, onMiss: (tok) => { this.cur.mistakes++; $('pzMsg').textContent = T('pz.ninja.missed', { item: L(tok) }); this.o.sfx?.mistake?.(); }, onEnd: () => this.solved() });
    this.ninja.start();
  }
  ...
  buildReflex(dish) {
    ...
    $('pzTitle').textContent = T('pz.reflex.title', { n: total }); $('pzAnswer').classList.remove('hidden');
    ...
    const tick = () => { if (over) return; const el = (performance.now() - t0) / 1000; const left = Math.max(0, limit - el); const bar = $('pzBar'); if (bar) { bar.style.width = `${(left / limit) * 100}%`; $('pzLeft').textContent = `${left.toFixed(1)}s`; } if (left <= 0) { over = true; this.cur.mistakes += need.size; $('pzMsg').textContent = T('pz.reflex.timeout', { list: [...need].map(L).join(', ') }); for (const b of $('pzGrid').children) { if (need.has(b.dataset.it)) b.classList.add('missed'); } this.o.sfx?.mistake?.(); setTimeout(() => this.solved(), 1400); return; } this._raf = requestAnimationFrame(tick); };
    ...
    $('pzGrid').replaceChildren(...this._grid.map((it) => {
      ...
      el.onclick = () => { if (over) return; this.cur.taps++; if (need.has(it)) { need.delete(it); got++; el.classList.add('ok'); el.disabled = true; $('pzGot').textContent = `${got}/${total}`; this.o.sfx?.place?.(); if (!need.size) { over = true; cancelAnimationFrame(this._raf); this.solved(); } } else this.wrong(el, T('pz.notIn', { item: L(it), dish: dishLabel(dish) })); };
      return el;
    }));
  ...
  buildAssemble() {
    ...
    const who = (dish) => { const r = REGULARS.filter((x) => x.dish === dish); if (r.length && Math.random() < 0.5) { const g = r[Math.floor(Math.random() * r.length)]; return { name: g.name, regular: g.id }; } return { name: T(`cust.walkin.${1 + Math.floor(Math.random() * 6)}`), regular: null }; };
    ...
    const place = (tok, el) => {   // đặt token vào tô của phiếu đang làm — đúng bước kế thì nhận, sai thì lỗi
      ...
      if (!ok) { wrongTap(el, t.k === 0 ? T('pz.startWith', { item: L(need) }) : T('pz.notYet', { item: L(tok) })); return false; }
    ...
    const serve = (t) => {
      ...
      $('pzMsg').innerHTML = `<span style="color:${t.mistakes ? 'var(--broth)' : '#2f8a3a'}">${t.name}: “${say}” · ${sec.toFixed(0)}s${t.mistakes ? ` · ${T('pz.nErr', { n: t.mistakes })}` : ` · ${T('pov.quality.perfect')}`}</span>`;
    ...
    const renderBowl = () => {
      ...
      if (!t) { box.innerHTML = `<span class="ans">${T('pz.waiting')}</span>`; return; }
    ...
    const noodleCard = (n) => {
      ...
      const draw = () => { el.innerHTML = `${icon(n)}<span>${L(n)}${st === 'hot' ? ` — ${T('pv.hot')}` : st === 'rinsed' ? ` — ${T('pv.rinsed')}` : st === 'hot2' ? ` — ${T('pv.hotAgain')}` : ''}</span>` + (st === 'raw' ? `<div class="opt"><button data-a="blanch">♨️ ${T('pz.act.blanch')}</button></div>` : st === 'hot' ? `<div class="opt"><button data-a="bowl">🥣 ${T('pz.act.bowl')}</button><button data-a="rinse">🚿 ${T('pz.act.rinse')}</button></div>` : st === 'rinsed' ? `<div class="opt"><button data-a="reblanch">♨️ ${T('pz.act.reblanch')}</button><button data-a="bowl">🥣 ${T('pz.act.bowl')}</button></div>` : `<div class="opt"><button data-a="bowl">🥣 ${T('pz.act.bowl')}</button></div>`); };
      ...
      el.addEventListener('click', (e) => {
        ...
        if (a === 'rinse') { if (wf(t.dish) === 'noodle-hot-only') return wrongTap(el, T('pz.onceOnly', { item: L(n) })); return act(600, 'rinsed'); }
        if (a === 'reblanch') return act(600, 'hot2');
        if (a === 'bowl') {
          const need = t.steps[t.k]; const tok = `noodle-drained:${n}`;
          if (wf(t.dish) === 'noodle-base' && st !== 'hot2') return wrongTap(el, st === 'hot' ? T('pz.needRinse') : T('pz.needReblanch'));
          if (need !== tok) return wrongTap(el, need.startsWith('noodle-drained:') ? T('pz.wrongNoodle', { want: L(need.split(':')[1]), item: L(n) }) : T('pz.noodleNotYet', { next: L(need) }));
    ...
    section(T('pz.sec.bowls'), bowls.map((b) => cardBtn(b, 'bw', (el) => { if (el.classList.contains('busy')) return; el.classList.add('busy'); A.o.sfx?.drop?.(); setTimeout(() => { el.classList.remove('busy'); place(`bowl-hot:${b}`, el); }, 500); })));
    section(T('pz.sec.pot'), noodles.map(noodleCard));
    section(T('pz.sec.toppings'), tops.map((it) => cardBtn(it, 'tp', (el) => place(it, el))));
    section(T('pz.sec.broth'), broths.map((t) => cardBtn(t, 'br', (el) => place(t, el))));
    $('pzTitle').textContent = T('pz.assemble.title'); $('pzTickets').classList.remove('hidden'); $('pzProgress').textContent = `0/${total}`;
    spawn(); renderBowl();
  ...
  solved() {
    ...
    $('pzMsg').textContent = c.mistakes ? T('pz.doneErr', { n: c.mistakes, s: sec.toFixed(0) }) : T('pz.doneClean', { s: sec.toFixed(0) }); this.o.sfx?.done?.();
